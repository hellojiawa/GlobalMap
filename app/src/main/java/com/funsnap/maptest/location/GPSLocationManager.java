package com.funsnap.maptest.location;
...
public class GPSLocationManager {
    ...
    public static int mGPSCount;
    private final GpsStatus.Listener statusListener = new GpsStatus.Listener() {
        public void onGpsStatusChanged(int event) { // GPS状态变化时的回调，如卫星数

            GpsStatus status = null;
            try {
                status = gpsLocationManager.getLocationManager().getGpsStatus(null); // 取当前状态
            } catch (Exception e) {

            }

            if (event == GpsStatus.GPS_EVENT_SATELLITE_STATUS && status != null) {
